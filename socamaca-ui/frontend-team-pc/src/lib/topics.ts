...
export const topics = {
  status: (c: TeamColor) => teamTopic(c, "status"), // NEW: self_hp/battery/mission_state/robot_destroyed
  ...
  enemyDetections: (c: TeamColor) => teamTopic(c, "enemy/detections"), // NEW
...
export interface TeamStatusMsg {
  self_hp: number;
  battery: number;
  ammo: number; // NEW: 남은 장전 탄수(0~MAX_AMMO)
  reload_remaining: number; // NEW: 재장전 남은 초(0이면 발사 가능)
