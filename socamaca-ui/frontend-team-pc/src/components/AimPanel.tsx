...
export function AimPanel({ ros, color }: { ros: ROSLIB.Ros; color: TeamColor }) {
  ...
  const status = useTopic<TeamStatusMsg>(ros, topics.status(color), "socamaca_msgs/TeamStatus");
  const detections = useTopic<DetectionsMsg>(ros, topics.enemyDetections(color), "socamaca_msgs/Detections");
