...
drawio(`flowchart_SOCAMACA (1).drawio`)에 나온 토픽은 그대로,
프론트 렌더링에 필요해서 이번 스캐폴딩에서 새로 정의한 토픽은 NEW 표시.
...
# ---- 팀 PC 관련 토픽 (drawio) ----
TEAM_STATUS_TOPIC = team_topic  # 사용: team_topic(color, "status") -> "/blue/status" (NEW, 원래 diagram은 개별 필드였음)
...
ENEMY_DETECTIONS_TOPIC = lambda c: team_topic(c, "enemy/detections")  # noqa: E731  NEW: bbox 목록(JSON)
...
JUDGE_DETECTIONS_TOPIC = "/judge/detections"  # NEW: robot_blue/robot_red/ball bbox 목록(JSON)

# NEW: 원래 diagram엔 self_hp/mission_state/battery가 "j_tstat"로 뭉뚱그려져 있어
