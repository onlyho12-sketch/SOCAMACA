...
# HitEvent/TeamStatus는 socamaca_msgs 패키지가 워크스페이스에 빌드되어 있어야 임포트된다
# (ros2_ws/src/socamaca_msgs). 없으면 HP 연동 없이 match_state publish만 동작하도록 degrade.
try:
    from socamaca_msgs.msg import HitEvent, TeamStatus

    _HAS_SOCAMACA_MSGS = True
except ImportError:
    _HAS_SOCAMACA_MSGS = False
    ...
    def __init__(self, on_hit: Optional[Callable[[str, int, int], None]] = None) -> None:
        ...
        if _HAS_SOCAMACA_MSGS:
            ...
            self.get_logger().warn(
                "socamaca_msgs를 찾을 수 없음 — /red|blue/status publish와 hit_event 구독을 건너뜀"
                " (ros2_ws/src/socamaca_msgs를 colcon build 후 source 했는지 확인)"
            )
