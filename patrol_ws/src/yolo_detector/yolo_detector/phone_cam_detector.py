...
- socamaca-ui 계약(topics.ts): 조준각은 aim/set_ori, 탐지 목록은 enemy/detections
...
from socamaca_msgs.msg import Detection, Detections
...
class PhoneCamDetector(Node):
    ...
    def inference_loop(self):
        while rclpy.ok():
            ...
            ui_detections = []  # socamaca-ui enemy/detections 용 (TARGET_CLASS 매칭된 것만)
            ...
            if (now - self.last_image_pub_time).nanoseconds / 1e9 >= self.image_pub_period:
                ...
                # socamaca-ui CameraFeed.tsx 용: 같은 프레임을 JPEG로 인코딩해서 발행.
